function createUnityInstance(t, r, d) {
    function i(e, t) {
        if (!i.aborted && r.showBanner) return "error" == t && (i.aborted = !0), r.showBanner(e, t);
        switch (t) {
            case "error":
                console.error(e);
                break;
    ...
    function n(e) {
        ...
        (r += "\n" + (n = n.startsWith(r) ? n.substring(r.length) : n).trim()) && c.stackTraceRegExp && c.stackTraceRegExp.test(r) && S(r, e.filename || t && (t.fileName || t.sourceURL) || "", e.lineno || t && (t.lineNumber || t.line) || 0)
    }
    ...
    }), window.addEventListener("error", n), window.addEventListener("unhandledrejection", n), c.deinitializers.push(function() {
        for (var e in c.disableAccessToMediaDevices(), a.forEach(function(e) {
                t.removeEventListener(e, s)
            }), window.removeEventListener("error", n), window.removeEventListener("unhandledrejection", n), c.intervals) window.clearInterval(e);
        c.intervals = {}
    ...
    var l, u, h, f, p, g, m, b, y = "",
        v = "",
        w = (document.addEventListener("webkitfullscreenchange", function(e) {
            document.webkitCurrentFullScreenElement === t ? t.style.width && (y = t.style.width, v = t.style.height, t.style.width = "100%", t.style.height = "100%") : y && (t.style.width = y, t.style.height = v, v = y = "")
        }), {
            Module: c,
    ...
    function x(e, t) {
        ...
        return e.replace(new RegExp(r, "g"), function(e) {
            e = e.match(n);
            return e[1] + (t[e[2]] ? t[e[2]] + "@" : "") + e[2] + e[3]
        })
    }

    function S(n, o, a) {
        c.symbols ? C(x(n, c.symbols), o, a) : c.symbolsUrl ? L("symbolsUrl").then(function(e) {
            for (var t = "", r = 0; r < e.length; r++) t += String.fromCharCode(e[r]);
            c.symbols = JSON.parse(t), C(x(n, c.symbols), o, a)
        }).catch(function(e) {
            C(n, o, a)
        }) : C(n, o, a)
    }
    ...
    function E(e, t) {
        if ("symbolsUrl" != e) {
            ...
            for (e in c.downloadProgress) {
                ...
                a++, r.lengthComputable ? (n += r.loaded, o += r.total, s++) : r.finished || i++
            }
            d(.9 * (a ? (a - i - (o ? s * (o - n) / o : 0)) / a : 0))
        }
    }
        ...
        o.isConnected = new Promise(function(t, r) {
            try {
                ...
                o.openDBTimeout = setTimeout(function() {
                    void 0 === o.database && r(new Error("Could not connect to database: Timeout."))
                }, 2e3);
                var e = f.open(l.name, l.version);
                e.onupgradeneeded = function(e) {
                    var t;
                    (e = (e = e).target.result).objectStoreNames.contains(h.name) || e.createObjectStore(h.name), e.objectStoreNames.contains(u.name) || (t = e.createObjectStore(u.name, {
                        keyPath: "url"
                    ...
                    n(), o.database = null, r(new Error("Could not connect to database."))
                }
            } catch (e) {
                n(), o.database = null, r(new Error("Could not connect to database."))
            }
    ...
    function k(e) {
        return k.link = k.link || document.createElement("a"), k.link.href = e, k.link.href
    }
    ...
    return c.SystemInfo = function() {
        ...
        return S(e, "", 0), !0
    }, Error.stackTraceLimit = Math.max(Error.stackTraceLimit || 0, 50), c.readBodyWithProgress = function(a, s, i) {
            ...
            function o() {
                return void 0 === e ? a.arrayBuffer().then(function(e) {
                    ...
                    if (e.done) {
                        ...
                        for (var t = new Uint8Array(h), r = (t.set(u, 0), f), n = 0; n < c.length; ++n) t.set(c[n], r), r += c[n].length;
                        return t
                    }
                    return h + e.value.length <= u.length ? (u.set(e.value, h), f = h + e.value.length) : c.push(e.value), h += e.value.length, s({
                        type: "progress",
                        response: a,
                        total: Math.max(l, h),
                        loaded: h,
        ...
        name: "WebAssembly",
        version: 1
    }, f = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB, T.UnityCacheDatabase = l, T.RequestStore = u, T.WebAssemblyStore = h, p = null, T.getInstance = function() {
        return p = p || new T
        ...
        return T.destroyInstance().then(function() {
            return new Promise(function(e, t) {
    ...
    }, T.prototype.execute = function(a, s, i) {
        return this.isConnected.then(function() {
            return new Promise(function(t, r) {
                try {
                    var e, n, o;
                    null === this.database ? r(new Error("indexedDB access denied")) : (e = -1 != ["put", "delete", "clear"].indexOf(s) ? "readwrite" : "readonly", n = this.database.transaction([a], e).objectStore(a), "openKeyCursor" == s && (n = n.index(i[0]), i = i.slice(1)), (o = n[s].apply(n, i)).onsuccess = function(e) {
                        t(e.target.result)
    ...
    }, T.prototype.storeRequest = function(e) {
        return this.execute(u.name, "put", [e])
    }, T.prototype.close = function() {
        return this.isConnected.then(function() {
            this.database && (this.database.close(), this.database = null)
        }.bind(this))
    }, T), c.cachedFetch = (g = c.UnityCache, m = g.RequestStore, b = c.fetchWithProgress, U.prototype.arrayBuffer = function() {
        return Promise.resolve(this.parsedBody.buffer)
    }, U.prototype.blob = function() {
        ...
        var e, t, a = g.getInstance(),
            r = k("string" == typeof n ? n : n.url),
            s = {
                enabled: (e = r, (!(t = o) || !t.method || "GET" === t.method) && ((!t || -1 != ["must-revalidate", "immutable"].indexOf(t.control)) && !!e.match("^https?://")))
            };

        function i(e, t) {
            return b(e, t).then(function(e) {
                if (s.enabled && !s.revalidated) {
                    if (304 === e.status) return s.result.revalidated = s.result.accessed, s.revalidated = !0, a.storeRequest(s.result).then(function() {
                        P("'" + s.result.url + "' successfully revalidated and served from the indexedDB cache")
                    }).catch(function(e) {
                        P("'" + s.result.url + "' successfully revalidated but not stored in the indexedDB cache due to the error: " + e)
                    }), new U(s.result.response);
                    200 == e.status ? (s.result = B(e.url, s.company, s.product, s.accessed, e), s.revalidated = !0, a.storeRequest(s.result).then(function() {
                        P("'" + s.result.url + "' successfully downloaded and stored in the indexedDB cache")
                    }).catch(function(e) {
                        P("'" + s.result.url + "' successfully downloaded but not stored in the indexedDB cache due to the error: " + e)
                    })) : P("'" + s.result.url + "' request failed with status: " + e.status + " " + e.statusText)
                }
                return e
            })
        }
