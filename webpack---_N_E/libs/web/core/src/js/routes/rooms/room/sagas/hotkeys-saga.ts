...
type HotkeyOptions = {
  ...
  ignoreIfModalOpen?: boolean
}

type HotkeyMap = Record<string, { handler: Saga<[KeyboardEvent, HotkeyOptions]>; options: HotkeyOptions }>
...
function* setupHotkeyListener(customKeybinds: Record<string, Keybind>) {
  ...
  for (const [key, keybind] of Object.entries(customKeybinds)) {
    hotkeyMap[key] = { handler: createCustomKeybindHandler(keybind), options: {} }
  }
